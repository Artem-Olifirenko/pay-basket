...
func TestMaxCost_Resolve(t *testing.T) {
	type args struct {
		ctx            context.Context
	...
	tests := []struct {
		...
		{
			name: "allowing cases for max payments",
			args: &args{
				...
				orders: func(ctrl *gomock.Controller) map[order.PaymentId]order.Order {
					amounts := map[order.PaymentId]int{
						...
						order.PaymentIdCardsOnline:            1000000,
						order.PaymentIdCredit:                 0,
						order.PaymentIdYandex:                 500000,
						order.PaymentIdTerminalOrCashbox:      15000,
						order.PaymentIdWebmoney:               60000,
						order.PaymentIdTerminalPinpad:         0,
						order.PaymentIdInstallments:           0,
						order.PaymentIdSberbankBusinessOnline: 0,
						order.PaymentIdSbp:                    1000000,
						order.PaymentIdCashWithCard:           0,
						order.PaymentIdApplePay:               1000000,
					}
		...
		{
			name: "disallowing cases for max payments",
			args: &args{
				ctx: context.Background(),
				resolvedIdsMap: payment.ResolvedPaymentIdMap{
					order.PaymentIdInvalid:           payment.NewResolvedPaymentId(order.PaymentIdInvalid),
				...
				orders: func(ctrl *gomock.Controller) map[order.PaymentId]order.Order {
					amounts := map[order.PaymentId]int{
						order.PaymentIdInvalid:           2,
						order.PaymentIdUnknown:           2,
						order.PaymentIdCardsOnline:       1000001,
						order.PaymentIdYandex:            500001,
						order.PaymentIdTerminalOrCashbox: 15001,
						order.PaymentIdWebmoney:          60001,
						order.PaymentIdSbp:               1000001,
						order.PaymentIdApplePay:          1000001,
					}
					orders := make(map[order.PaymentId]order.Order)
					for paymentId, amount := range amounts {
						orderMock := order.NewMockOrder(ctrl)
			...
			want: func() (order.AllowStatus, *order.DisallowReasonWithInfo, error) {
				return order.AllowStatusLimited, order.NewDisallowReasonWithInfo(order.DisallowReasonPaymentIdMaxCost, order.SubsystemBasket), nil
			},
		},
		{
			name: "error case",
...
func TestCountPaymentTypesForMax(t *testing.T) {
	const countOfPayments = 14
	var paymentTypesCounter int
	for _, p := range order.PaymentIds {
		if p.IsValid() {
			paymentTypesCounter++
		}
	}
	assert.GreaterOrEqualf(t, paymentTypesCounter, countOfPayments, "please, add information about new payment type(s) to test")
	assert.LessOrEqualf(t, paymentTypesCounter, countOfPayments, "please, delete information about removed payment type(s) from test")
}
