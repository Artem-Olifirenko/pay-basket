...
type ProductClient interface {
	FilterServices(ctx context.Context, in *productv1.FilterServicesRequest, opts ...grpc.CallOption) (*productv1.FilterServicesResponse, error)
}

type SubcontractServiceForProductItemFactory struct {
	productClient ProductClient
}

func NewSubcontractServiceForProductItemFactory(productClient productv1.ProductAPIClient) *SubcontractServiceForProductItemFactory {
	return &SubcontractServiceForProductItemFactory{productClient: productClient}
}

func (i *SubcontractServiceForProductItemFactory) Creatable(itemType basket_item.Type) bool {
	return itemType == basket_item.TypeSubcontractServiceForProduct
}

func (i *SubcontractServiceForProductItemFactory) Create(
	ctx context.Context,
	...
	if parentItem == nil {
		return nil, fmt.Errorf("parent item is neccessary for '%s'", basket_item.TypeSubcontractServiceForProduct)
	}
	...
	if len(response.GetProductServices()) == 0 {
		return nil, internal.NewNotFoundError(
			fmt.Errorf("subcontract services not found for product %s in space %s",
				parentItem.ItemId(),
				parentItem.SpaceId()))
	}
...
searchServiceLoop:
	...
	if service == nil {
		return nil, internal.NewNotFoundError(
			fmt.Errorf("subcontract services not found for product %s in space %s with id %s",
				parentItem.ItemId(),
				parentItem.SpaceId(),
				itemId))
	}
	...
	subcontractServiceItem := basket_item.NewItem(
		itemId,
	...
	if price == 0 {
		subcontractServiceItem.AddProblem(basket_item.NewProblem(basket_item.ProblemNotAvailable,
			"услугу купить невозможно"))
	}

	subcontractServiceItem.Additions().SetSubcontractServiceForProduct(&basket_item.SubcontractItemAdditions{})

	subcontractServiceItem.Additions().SetService(basket_item.NewService(
		service.GetIsAvailForCredit(),
		service.GetIsAvailForInstallments()))

	err = parentItem.AddChild(subcontractServiceItem)
	if err != nil {
		return nil, fmt.Errorf("can't make item '%s' child of item '%s'", subcontractServiceItem.Type(), parentItem.Type())
	}

	return subcontractServiceItem, nil
}
